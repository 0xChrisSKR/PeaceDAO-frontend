...
export function useI18n() {
  const [lang, setLang] = useState<'zh'|'en'>(typeof window !== 'undefined' && (localStorage.getItem('lang') as 'zh'|'en') || 'zh');
  const t = (k: keyof typeof dict['zh'], fallback?: string) => (dict[lang][k] as string) || fallback || k;
  const toggle = () => { const next = lang==='zh'?'en':'zh'; setLang(next); if (typeof window !== 'undefined') localStorage.setItem('lang', next); };
